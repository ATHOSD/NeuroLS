#!/usr/bin/env python3
"""
Segmentation script for CHOA-Fetal Chiari dataset using MoE_foundation_v3.

Input:
  /labs/wanglab/projects/lifespan-T1T2FA/CHOA-Fetal/Disease/chiari/recon/
  Structure: {scan_id}/recon.nii.gz

Output (mirrors input structure):
  /opt/localdata/data/usr-envs/ruiying/Code/foundation/result_tissue/choa_disease/chiari/

Demographics:
  /labs/wanglab/projects/lifespan-T1T2FA/CHOA-Fetal/Disease/chiari/chiari_demographics.csv
  columns: sub, sex, GA, scan_id, chiari_classification

All subjects are fetal age group, all scans are T2w.

Usage:
    python test_segmentation_choa_chiari.py [--checkpoint <path>] [--resume]
"""
...
INPUT_DIR      = Path("/labs/wanglab/projects/lifespan-T1T2FA/CHOA-Fetal/Disease/ventri/recon")
OUTPUT_DIR     = Path("/opt/localdata/data/usr-envs/ruiying/Code/foundation/result_tissue/choa_disease/ventri")
DEMO_PATH      = Path("/labs/wanglab/projects/lifespan-T1T2FA/CHOA-Fetal/Disease/ventri/ventri_demographics.csv")
...
# Fixed for all Chiari scans
FIXED_AGE_GROUP = 'fetal'
FIXED_MODALITY  = 'T2w'
...
def load_demo_lookup(demo_path: Path) -> dict:
    """Return {scan_id_str: {'sub', 'sex', 'GA', 'chiari_classification'}} ."""
    ...
    for _, row in df.iterrows():
        # scan_id stored as float in CSV (e.g. 650966222452816.0); dir name is integer string
        try:
            scan_key = str(int(float(row['scan_id'])))
        except (ValueError, TypeError):
            continue
        lookup[scan_key] = {
            'sub':                    str(row['sub']).strip(),
            'sex':                    str(row['sex']).strip()  if pd.notna(row['sex'])  else None,
            'GA':                     float(row['GA'])         if pd.notna(row['GA'])   else None,
...
def process_chiari(cfg, model, resume):
    print(f"\n{'='*60}")
    print(f"Dataset: CHOA-Fetal Chiari")
    ...
    # Collect all recon.nii.gz files; each lives in INPUT_DIR/{scan_id}/recon.nii.gz
    entries = sorted(INPUT_DIR.glob("*/recon.nii.gz"))
    print(f"  Recon images found: {len(entries)}")

    skipped_exist = 0
    skipped_err   = 0
    processed     = 0

    for img_path in tqdm(entries, desc="Chiari"):
        scan_id  = img_path.parent.name          # e.g. "101650918679031"
        out_path = OUTPUT_DIR / scan_id / "recon.nii.gz"
        ...
        demo = demo_lookup.get(scan_id, {})
        if not demo:
            tqdm.write(f"  [WARN] {scan_id}: not in demographics, processing anyway")
...
def main():
    parser = argparse.ArgumentParser(
        description='Segment CHOA-Fetal Chiari dataset (MoE_v3) — T2w fetal')
    ...
    p, se, serr = process_chiari(cfg, model, resume=args.resume)
