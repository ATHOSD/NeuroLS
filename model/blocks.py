...
'''
This record some basic blocks implementation
'''
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='gcr', num_groups=8, padding=1):
        ...
        self.add_module('SingleConv1',
                        SingleConv(conv1_in_channels, conv1_out_channels, kernel_size, order=order, num_groups=num_groups,
                                   padding=padding))
        # conv2
        self.add_module('SingleConv2',
                        SingleConv(conv2_in_channels, conv2_out_channels, kernel_size, order=order, num_groups=num_groups,
                                   padding=padding))
...
class ExtResNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride_size=1, order='cge', num_groups=8, **kwargs):
        ...
        if stride_size > 1:
            self.conv1 = SingleConv(in_channels, out_channels, kernel_size=3,
                                    stride_size=stride_size, order=order, num_groups=num_groups, padding=1)
        else:
